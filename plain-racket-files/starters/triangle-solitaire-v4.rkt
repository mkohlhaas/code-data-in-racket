...
;; <template as composition of map and filter>
(define (next-boards bd)
  (local [;; Jump -> Board
          ;; <template according to Jump>
          (define (do-jump j)
            (local [(define from (jump-from j))
                    (define over (jump-over j))
                    (define to   (jump-to   j))]
              
              (set-cell (set-cell (set-cell bd to true) over false) from false)))
...
;; <template as composition of foldr and map>
#;
(define (solved? bd)
  (= 1 (foldr + 0 (map (lambda (v) (if v 1 0)) bd))))


;; <template as foldr of map of filter>
(define (solved? bd)
  (= 1
     (foldr + 0
            (map (lambda (x) 1)
